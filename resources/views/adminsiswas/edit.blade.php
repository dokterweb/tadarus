    ...
    <div class="row">
        <div class="col-md-4">
            <div class="card card-primary">
                ...
                <form method="POST" action="{{route('adminsiswas.update',$adminsiswa->id)}}">
                    @csrf
                    @method('PUT')
                    <div class="card-body">
                        <div class="form-group">
                            <label>Nama Admin</label>
                            <input type="text" name="name" class="form-control" value="{{$adminsiswa->user->name}}">
                            @error('name')
                        ...
                        <div class="form-group">
                            ...
                            <select name="jenis" class="form-control" style="width:100%">
                                <option value="putra" {{ $adminsiswa->jenis == 'putra' ? 'selected' : '' }}>Putra</option>
                                <option value="putri" {{ $adminsiswa->jenis == 'putri' ? 'selected' : '' }}>Putri</option>
                            </select>
                            @error('jenis')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group">
                            <label >Email</label>
                            <input type="text" name="email" class="form-control" value="{{$adminsiswa->user->email}}">
                            @error('email')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
