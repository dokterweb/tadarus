...
@section('content')
    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('sabqis.history.update', $history->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                    <div class="col-md-4">
                        <div class="form-group">
                            <label for="tgl_sabqi">Tanggal Muroja'ah</label>
                            <input type="date" name="tgl_sabqi" class="form-control" value="{{ $history->tgl_sabqi }}" required>
                        </div>
                    </div>
                    <div class="col-md-8">
                        <div class="form-group">
...
<script>
    $(document).ready(function () {
        $('#edit_sura_no').change(function () {
            ...
            if (selectedSuraNo) {
                $.ajax({
                    url: '/get-surat-details/' + selectedSuraNo,  // Pastikan URL benar
                    type: 'GET',
                    dataType: 'json',
